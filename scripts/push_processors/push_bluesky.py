...
def parse_bluesky_config(file_path):
    ...
    for line in lines:
        ...
        # Check if the key is 'bluesky_username' or 'bluesky_app_password'
        if key == 'bluesky_username':
            bluesky_username = value.strip('\'"')
        elif key == 'bluesky_app_password':
            bluesky_app_password = value.strip('\'"')

    return bluesky_username, bluesky_app_password

config_path = os.path.expanduser("~/.bluesky.conf")

BLUESKY_USERNAME, BLUESKY_APP_PASSWORD = parse_bluesky_config(config_path)
...
for image_group in images:
  # create post
  client.send_images(text=post_text, images=image_group)